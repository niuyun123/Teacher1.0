 ...
 * 作者单位：四川汉博德信息技术有限公司
 * 创建时间：2016/8/4 0004 13:28
 */
public class WheelView extends ScrollView {
    public static final String TAG = WheelView.class.getSimpleName();

    public static class OnWheelViewListener {
        public void onSelected(int selectedIndex, Pickers item) {
    ...
    private List<Pickers> getItems() {
        return items;
    }

    public void setItems(List<Pickers> list) {
        if (null == items) {
            items = new ArrayList<Pickers>();
        }
        items.clear();
        items.addAll(list);
        ...
        for (int i = 0; i < offset; i++) {
            items.add(0, new Pickers("",""));
            items.add(new Pickers("",""));
        }
...
//                    Log.d(TAG, "remainder: " + remainder + ", divided: " + divided);
    ...
    private void initData() {
        ...
        for (Pickers item : items) {
            views.addView(createView(item.getShowConetnt()));
        }
